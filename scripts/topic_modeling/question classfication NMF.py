...
if USE_FULL_DATA:
    ...
    df_sample = df.copy()
    print("Using FULL dataset:", len(df_sample))

elif N_SAMPLE is not None and len(df) > N_SAMPLE:
    ...
    df_sample = df.copy()
    print("Using all data:", len(df_sample))


# ============================================================
# 5. Text preprocessing (baseline: keep code)
# ============================================================
...
print(f"Cleaning took {time.time() - t0:.1f}s")
...
print(f"Building TF-IDF took {time.time() - t0:.1f}s")


# ============================================================
# 7. Run NMF for K = 2 to 20
# ============================================================
...
for K in K_VALUES:
    ...
    print(f"NMF K={K} took {train_time:.1f}s")
    ...
    for topic_idx in range(K):
        ...
        for rank, word_idx in enumerate(top_indices, start=1):
            ...
            topic_rows.append({
                ...
                "weight": H[topic_idx, word_idx]
            })

    topic_df = pd.DataFrame(topic_rows)

    topic_df.to_csv(
        os.path.join(OUTPUT_DIR, f"nmf_K{K}_top_words.csv"),
        index=False
    )

    # ---- Document-topic assignments ----
    topic_proportions = W / (W.sum(axis=1, keepdims=True) + 1e-12)

    dominant_topic = topic_proportions.argmax(axis=1)
    dominant_probability = topic_proportions.max(axis=1)

    assignments = pd.DataFrame({
        "Id": df_sample["question_id"].values,
        "dominant_topic": dominant_topic,
        "topic_weight": dominant_probability
    })

    assignments.to_csv(
        os.path.join(OUTPUT_DIR, f"nmf_K{K}_assignments.csv"),
        index=False
    )
    ...
    dominant_prob = topic_props.max(axis=1)

    mean_dom = dominant_prob.mean()
    std_dom = dominant_prob.std()
    frac_ambiguous = (dominant_prob < 0.5).mean()

    dominant_topic_arr = topic_props.argmax(axis=1)
    topic_counts = np.bincount(dominant_topic_arr, minlength=K)
...
eval_df.to_csv(
    os.path.join(OUTPUT_DIR, "nmf_K_evaluation.csv"),
    index=False
)

print("\n" + "=" * 70)
print("EVALUATION SUMMARY (K = 2 to 20)")
print("=" * 70)
