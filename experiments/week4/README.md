# Week 4 experiment scripts

Run commands from this directory. These scripts cover the second-round first-answer helpfulness pilot, encoder ablation, and subsequent question-intent prompt experiments. Taxonomy discussion and interpretation belong in the reports.

## Rebuild the two figures

```sh
python -m pip install numpy matplotlib
python make_intent_figures.py
```

This reads `report_outputs/figure_data.json` and regenerates the PNG/SVG figures without a GPU or inference. The JSON contains aggregate counts and original log hashes. Output validity and synthetic constraint compliance are **not** accuracy against independently established real-question labels. Invalid outputs are not negative labels.
...
```sh
python prepare_helpfulness.py --processed-dir /path/to/processed --out data
python prepare_intent_round3.py --pairs data/blinded_pairs.json --model-sample /path/to/model_sample_60000.jsonl --out intent_round3
python run_helpfulness_v3.py --data data/blinded_pairs.json --model /path/to/Qwen2.5-Coder-7B-Instruct --out results/helpfulness_7b
python run_intent_round3.py --data intent_round3/cases.json --model /path/to/Qwen2.5-Coder-7B-Instruct --out intent_round3_downloaded/results/7b-examples --arm examples
python run_intent_round4.py --data intent_round3/cases.json --model /path/to/Qwen2.5-Coder-7B-Instruct --out intent_round4_downloaded/results/7b-shared_json --arm examples --variant shared_json
```
...
```sh
python run_encoder_ablation.py --data /path/to/model_sample_60000.jsonl --minilm /path/to/all-MiniLM-L6-v2 --codebert /path/to/codebert-base --out results/encoder_60000 --n 60000 --batch-size 16
```
